...
export default async function MerchantLayout({
  ...
  const items: [string, string][] = [
    ["/merchant", t.merchant.nav.overview],
    ["/merchant/printers", t.merchant.nav.printers],
  ];
    ...
    <div className="min-h-screen">
      <header className="border-b border-slate-200 bg-white">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
          <Link href="/merchant" className="flex items-center gap-2 font-semibold text-slate-900">
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-md bg-brand-500 text-white text-xs">
              ХҮ
            </span>
            <span>{t.brand}</span>
            <span className="chip bg-amber-50 text-amber-700">Merchant</span>
          </Link>
